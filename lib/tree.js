...
function _queue (root) {
  ...
  return { queue: queue, childPos: childPos, parentPos: parentPos };
}
...
class Tree {
  ...
  /**
   * Walks the tree starting from the root and proceeding to this leaves.
   * A callback function is called on each node being visited, allowing to transform
   * nodes if necessary. The callback function is called with 2 arguments:
   *
   *  - Current node being visited
   *  - Transformed parent of the current node
   *
   * If the returned value of the callback is not `undefined`, the node is replaced
   * with this value and is supplied in the transformed form to the callback on the children.
   */
  walkFromRoot (fn) {
    var q = _queue(this);
    var queue = q.queue;
    ...
    for (var i = 0; i < queue.length; i++) {
      var result = fn(queue[i], queue[q.parentPos[i]]);
      if (result !== undefined) queue[i] = result;
    }
    return queue[0];
  }
